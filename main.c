...
int main(int argc, char **argv){
    ...
    for(int i = 1; i < argc; i++){
        if(my_strcmp(argv[i], "-h") == 0 || my_strcmp(argv[i], "--help") == 0){
            printHelp(argv[0]);
            return 0;
        } else if(my_strcmp(argv[i], "--gen-test") == 0){
            if(i + 1 < argc){
                gen_test = atoi(argv[++i]);
                if(gen_test < 0) gen_test = 0;
            }
        } else {
            dbfile = argv[i];
        }
        ...
        switch(choice){
            ...
            case 5: {
                ...
                if(fc == 1){
                    ...
                    printf("Min price: ");
                    int mn = 0;
                    if(scanf("%d", &mn) != 1) mn = 0;
                    while(getchar() != '\n');

                    printf("Max price: ");
                    int mx = 100000000;
                    if(scanf("%d", &mx) != 1) mx = 100000000;
                    while(getchar() != '\n');
