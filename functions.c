...
void addCarInteractive(void){
    ...
    printf("Technical Inspection: ");
    newc.technical_inspection = readLine();

    printf("Body Type: ");
    newc.body_type = readLine();

    printf("Mileage: ");
    newc.mileage = readLine();

    printf("Gear Box: ");
    newc.gear_box = readLine();

    printf("Fuel Type: ");
    newc.fuel_type = readLine();
...
void sortCarsByPrice(void){
    qsort(cars, car_count, sizeof(Car), cmp_price);
    printf("Sorted by price.\n");
}

void sortCarsByYear(void){
    qsort(cars, car_count, sizeof(Car), cmp_year);
    printf("Sorted by year.\n");
}

void sortCarsById(void){
    qsort(cars, car_count, sizeof(Car), cmp_id);
    printf("Sorted by id.\n");
}
...
int loadFromFile(const char *filename){
    ...
    for(int i = 0; i < count; i++){
        ...
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.color = my_strdup(buf);

        if(!fgets(buf, sizeof(buf), f)) break;
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.technical_inspection = my_strdup(buf);

        if(!fgets(buf, sizeof(buf), f)) break;
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.body_type = my_strdup(buf);

        if(!fgets(buf, sizeof(buf), f)) break;
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.mileage = my_strdup(buf);

        if(!fgets(buf, sizeof(buf), f)) break;
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.gear_box = my_strdup(buf);

        if(!fgets(buf, sizeof(buf), f)) break;
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.fuel_type = my_strdup(buf);

        if(!fgets(buf, sizeof(buf), f)) break;
        ln = my_strlen(buf);
        if(ln > 0 && buf[ln - 1] == '\n') buf[ln - 1] = 0;
        tmp.owner = my_strdup(buf);
